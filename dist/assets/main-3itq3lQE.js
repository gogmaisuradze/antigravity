document.addEventListener("DOMContentLoaded",()=>{y(),v(),h(),w(),L(),k(),E(),B(),A(),I(),q()});function y(){const e=document.getElementById("mobile-menu-btn"),t=document.getElementById("mobile-menu");e&&t&&(e.addEventListener("click",o=>{o.stopPropagation(),t.classList.toggle("hidden");const a=e.querySelector("span");a&&(t.classList.contains("hidden")?a.textContent="menu":a.textContent="close")}),document.addEventListener("click",o=>{if(!t.contains(o.target)&&o.target!==e){t.classList.add("hidden");const a=e.querySelector("span");a&&(a.textContent="menu")}}))}function v(){document.body.insertAdjacentHTML("beforeend",`
  ...
  `);const t=document.getElementById("booking-modal"),o=document.getElementById("booking-modal-card"),a=document.getElementById("close-modal-btn");document.addEventListener("click",n=>{n.target&&(n.target.classList.contains("booking-btn")||n.target.closest(".booking-btn"))&&(n.preventDefault(),c())});function c(){t.classList.remove("opacity-0","pointer-events-none"),o.classList.remove("scale-95"),o.classList.add("scale-100")}function r(){t.classList.add("opacity-0","pointer-events-none"),o.classList.remove("scale-100"),o.classList.add("scale-95")}a&&a.addEventListener("click",r),t.addEventListener("click",n=>{n.target===t&&r()}),document.addEventListener("keydown",n=>{n.key==="Escape"&&r()})}function h(){const e=document.getElementById("blog-categories"),t=document.getElementById("blog-search"),o=document.getElementById("blog-grid");if(!o)return;const a=Array.from(o.querySelectorAll(".blog-post"));let c="all",r="";o.insertAdjacentHTML("beforeend",`
  ...
  `);const s=document.getElementById("no-results-panel");if(e){const u=e.querySelectorAll(".category-filter");u.forEach(i=>{i.addEventListener("click",()=>{u.forEach(d=>{d.className="category-filter text-on-surface-variant hover:text-primary transition-colors cursor-pointer"}),i.className="category-filter text-secondary border-b border-secondary pb-1 cursor-pointer hover:opacity-85 transition-opacity",c=i.getAttribute("data-category"),l()})})}t&&t.addEventListener("input",u=>{r=u.target.value.toLowerCase().trim(),l()});function l(){let u=0;a.forEach(i=>{const d=i.getAttribute("data-category"),p=i.querySelector("h3").textContent.toLowerCase(),f=i.querySelector("p").textContent.toLowerCase(),g=p.includes(r)||f.includes(r);(c==="all"||d===c)&&g?(i.style.display="flex",i.classList.add("animate-fade-in"),u++):(i.style.display="none",i.classList.remove("animate-fade-in"))}),u===0?s.classList.remove("hidden"):s.classList.add("hidden")}}function w(){document.addEventListener("invalid",n=>{(n.target.tagName==="INPUT"||n.target.tagName==="TEXTAREA"||n.target.tagName==="SELECT")&&n.target.setCustomValidity("გთხოვთ სწორად შეავსოთ ველი")},!0),document.addEventListener("input",n=>{(n.target.tagName==="INPUT"||n.target.tagName==="TEXTAREA"||n.target.tagName==="SELECT")&&n.target.setCustomValidity("")}),document.addEventListener("change",n=>{n.target.tagName==="SELECT"&&n.target.setCustomValidity("")}),document.body.insertAdjacentHTML("beforeend",`
  ...
  `);const t=document.getElementById("toast-container"),o=document.getElementById("toast-message");function a(n){n&&(o.textContent=n),t.classList.remove("translate-y-24","opacity-0","pointer-events-none"),t.classList.add("translate-y-0","opacity-100"),setTimeout(()=>{t.classList.remove("translate-y-0","opacity-100"),t.classList.add("translate-y-24","opacity-0","pointer-events-none")},4500)}const c="8563426842:AAEuhg8EXmAV18NXtlAaiky0ZzWGvNXkJQU",r="443575738";document.addEventListener("submit",n=>{if(n.target&&(n.target.classList.contains("contact-form")||n.target.id==="booking-modal-form")){n.preventDefault();const s=n.target,l=s.querySelector('input[type="tel"]');if(l&&l.value&&l.value.replace(/\s+/g,"").length<5){alert("გთხოვთ მიუთითოთ სწორი ტელეფონის ნომერი");return}const u=s.querySelector('input[type="text"]'),i=s.querySelector('input[type="tel"]'),d=s.querySelector('input[type="date"]'),p=s.querySelector("select"),f=s.querySelector("textarea"),g={name:u?u.value:"",phone:i?i.value:"",date:d?d.value:"N/A",service:p?p.value:"ინდივიდუალური თერაპია",message:f?f.value:"",timestamp:new Date().toISOString(),sourceUrl:window.location.href};{const m=`🔔 *ახალი ჯავშანი საიტიდან!* 📅

👤 *სახელი:* ${g.name}
📞 *ტელეფონი:* ${g.phone}
📅 *თარიღი:* ${g.date}
💼 *სერვისი:* ${g.service}
✉️ *შეტყობინება:* ${g.message||"ცარიელი"}

🔗 *გვერდი:* ${g.sourceUrl}`;fetch(`https://api.telegram.org/bot${c}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:r,text:m,parse_mode:"Markdown"})}).then(b=>{b.ok||console.warn("Telegram Bot API response was not ok")}).catch(b=>{console.error("Error sending message to Telegram:",b)})}const x=document.getElementById("booking-modal");if(x&&!x.classList.contains("opacity-0")){const m=document.getElementById("close-modal-btn");m&&m.click(),a("ჯავშანი წარმატებით მიღებულია! ჩვენი ადმინისტრატორი მალე დაგიკავშირდებათ.")}else a("შეტყობინება წარმატებით გაიგზავნა! ჩვენ მალე დაგიკავშირდებით.");s.reset()}})}function L(){const e=document.createElement("div");e.className="glow-blob",e.style.background="radial-gradient(circle, rgba(241,191,98,0.2) 0%, rgba(18,20,22,0) 70%)",e.style.width="600px",e.style.height="600px",e.style.left="-100px",e.style.top="100px",e.style.position="fixed";const t=document.createElement("div");t.className="glow-blob",t.style.background="radial-gradient(circle, rgba(47,57,86,0.35) 0%, rgba(18,20,22,0) 70%)",t.style.width="800px",t.style.height="800px",t.style.right="-200px",t.style.bottom="-100px",t.style.position="fixed",document.body.appendChild(e),document.body.appendChild(t);let o=0;function a(){o+=.001;const r=Math.sin(o)*60,n=Math.cos(o)*40,s=Math.cos(o*1.3)*50,l=Math.sin(o*1.3)*70;e.style.transform=`translate(${r}px, ${n}px)`,t.style.transform=`translate(${s}px, ${l}px)`,requestAnimationFrame(a)}if(a(),window.innerWidth>768){let i=function(){r+=(s-r)*u,n+=(l-n)*u;const d=Math.sin(o)*30,p=Math.cos(o)*20;e.style.transform=`translate(${r+d}px, ${n+p}px)`,requestAnimationFrame(i)};var c=i;let r=0,n=0,s=0,l=0;const u=.05;document.addEventListener("mousemove",d=>{s=(d.clientX-window.innerWidth/2)*.15,l=(d.clientY-window.innerHeight/2)*.15}),i()}}function k(){const e=document.querySelectorAll(".reveal-hidden"),t=new IntersectionObserver(o=>{o.forEach(a=>{a.isIntersecting&&(a.target.classList.add("reveal-visible"),t.unobserve(a.target))})},{threshold:.05,rootMargin:"0px 0px -40px 0px"});e.forEach(o=>t.observe(o))}function E(){const e=document.getElementById("main-nav");e&&(window.addEventListener("scroll",()=>{window.scrollY>30?e.classList.add("nav-scrolled"):e.classList.remove("nav-scrolled")}),window.scrollY>30&&e.classList.add("nav-scrolled"))}function B(){const e=document.querySelectorAll(".methodology-pill"),t=document.getElementById("methodology-detail-card");if(!t||e.length===0)return;const o={awareness:{title:"გაცნობიერება",subtitle:"Awareness & Introspection",icon:"psychology",text:"პირველი ნაბიჯი ყოველთვის შინაგანი მდგომარეობის გაცნობიერებაა. კარმალოგიკის პრინციპებით, ჩვენ ვაკვირდებით იმ ქვეცნობიერ პროგრამებსა და ქცევებს, რომლებიც განსაზღვრავენ ჩვენს ყოველდღიურობას. საკუთარი თავის სიღრმისეული დაკვირვება იძლევა უნიკალურ ძალას შინაგანი ტრავმების ტრანსფორმაციისთვის.",quote:"„სანამ ქვეცნობიერს გააცნობიერებდე, ის მართავს შენს ცხოვრებას და შენ მას ბედისწერას უწოდებ.“ — კარლ გუსტავ იუნგი"},harmony:{title:"ჰარმონია",subtitle:"Mind-Body Integration",icon:"balance",text:"სხეულისა და გონების ჰარმონიული ერთობა. ჩვენი მიდგომა ორიენტირებულია იმაზე, რომ ადამიანმა აღმოაჩინოს ემოციური წონასწორობა. ეს მიიღწევა კოგნიტური და ეგზისტენციალური პრაქტიკების სინთეზით, რაც საგრძნობლად ამცირებს შფოთვას და გვეხმარება აწმყო მომენტში დაბრუნებაში.",quote:"„ცხოვრება არის ბალანსი გამკლავებასა და გაშვებას შორის.“"},transformation:{title:"ტრანსფორმაცია",subtitle:"Crisis to Growth",icon:"history_edu",text:"ცვლილება გარდაუვალია, მაგრამ გაცნობიერებული ტრანსფორმაცია — შეგნებული არჩევანია. ჩვენ ვეხმარებით კლიენტებს, გადალახონ ცხოვრებისეული კრიზისული პერიოდები და გარდაქმნან დაგროვილი ემოციური ტკივილი ახალ შინაგან ძალად და შემოქმედებით რესურსად.",quote:"„ყოველი დიდი ტრანსფორმაცია იწყება ძველი რეალობის ქაოსით, მაგრამ მთავრდება სულიერი სიცხადით.“"},resilience:{title:"მდგრადობა",subtitle:"Mental Resilience",icon:"diamond",text:"მყარი მენტალური საყრდენის შექმნა. ჩვენი თერაპიის უმთავრესი მიზანია კლიენტის მენტალური ჰიგიენის დამოუკიდებელი მართვა, რათა მომავალში მან მარტივად შეძლოს ცხოვრებისეული წნეხისადმი მედეგობისა და ფსიქოლოგიური სიმტკიცის შენარჩუნება.",quote:"„მდგრადობა არ ნიშნავს იმას, რომ არასოდეს დაეცემი. ეს ნიშნავს იმას, რომ ყოველთვის შეძლებ წამოდგომას.“"}};e.forEach(a=>{a.addEventListener("click",()=>{const c=a.getAttribute("data-methodology");o[c]&&(e.forEach(r=>{r.classList.remove("bg-secondary","text-on-secondary"),r.classList.add("bg-surface-container","text-on-surface-variant","hover:text-white")}),a.classList.remove("bg-surface-container","text-on-surface-variant","hover:text-white"),a.classList.add("bg-secondary","text-on-secondary"),t.style.opacity="0",t.style.transform="translateY(10px)",setTimeout(()=>{const r=o[c];t.innerHTML=`
          <div class="flex items-center gap-4 mb-6">
            <span class="material-symbols-outlined text-secondary text-4xl" style="font-variation-settings: 'FILL' 1;">${r.icon}</span>
            <div>
              <h3 class="text-2xl font-headline text-white">${r.title}</h3>
              <p class="text-xs text-outline-variant tracking-wider uppercase">${r.subtitle}</p>
            </div>
          </div>
          <p class="text-on-surface-variant leading-relaxed text-sm md:text-base font-light mb-8">${r.text}</p>
          <div class="border-t border-outline-variant/10 pt-6">
            <p class="text-secondary italic font-headline text-base md:text-lg">${r.quote}</p>
          </div>
        `,t.style.opacity="1",t.style.transform="translateY(0)"},250))})})}function A(){const e=document.querySelectorAll(".faq-item");e.length!==0&&e.forEach(t=>{const o=t.querySelector(".faq-trigger"),a=t.querySelector(".faq-content"),c=t.querySelector(".faq-icon");o&&a&&c&&o.addEventListener("click",()=>{const r=a.style.maxHeight===""||a.style.maxHeight==="0px";e.forEach(n=>{const s=n.querySelector(".faq-content"),l=n.querySelector(".faq-icon");s&&s!==a&&(s.style.maxHeight="0px",s.style.opacity="0",n.classList.remove("bg-surface-container-high/40"),l&&(l.style.transform="rotate(0deg)"))}),r?(a.style.maxHeight=a.scrollHeight+"px",a.style.opacity="1",t.classList.add("bg-surface-container-high/40"),c.style.transform="rotate(180deg)"):(a.style.maxHeight="0px",a.style.opacity="0",t.classList.remove("bg-surface-container-high/40"),c.style.transform="rotate(0deg)")})})}function I(){document.body.insertAdjacentHTML("beforeend",`
